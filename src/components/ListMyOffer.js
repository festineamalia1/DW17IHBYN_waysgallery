...
import { Container, Row, Col, Image, Jumbotron, Button, Table, Modal } from "react-bootstrap";
...
const ListMyOffer = () => {
 ...
 const { id } = state.user;
  const { isLoading, error, data: hired, refetch } = useQuery("getphoto", () =>
    API.get(`/hired/${id}`)
  );
              ...
              {hired.status == "Approved" ? (
                <td style={{ textAlign: "center" }}>
                  <button
                    ...
                    onClick={() => history.push(`/projek/${hired.id}`)}
                  >
                    send project
                  </button>
                </td>
